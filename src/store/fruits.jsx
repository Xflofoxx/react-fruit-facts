const fruitFacts = {
    apple: [
        'The average person eats 65 apples per year',
        'Apple seeds are mildly poisonous, but not enough to be dangerous to humans',
        'There are over 10,000 varieties of apples grown around the world',
        "25% of an apple's volume is air, that’s why they float",
        'Over 60 million tons of apples are grown worldwide annually'
    ],
    banana: [
        'Banana plants are not trees, they are actually giant herbs',
        'The inside of a banana peel can be used to polish leather shoes',
        'You can rub the inside of a banana skin on mosquito bites to stop the itchiness',
        'Bananas contain B6, which regulates blood glucose levels and your mood',
        'Research shows that eating bananas can assist learning',
        'Bananas are high in the B-complex vitamins, which help calm the nervous system',
        'Bananas can help fight depression, as they increase levels of serotonin',
        'Expecting mothers can eat bananas between meals to avoid morning sickness'
    ],
    coconut: [
        'In some parts of the world, trained monkeys are used to harvest coconuts',
        'The coconut palm is sometimes called the “tree of life”',
        'In Kerala in South India, coconut flowers must be present during a marriage ceremony'
    ],
    kiwi: [
        'People allergic to latex are highly likely to also be allergic to kiwi fruit',
        'Kiwi fruit contains actinidin which can be used to tenderize meat'
    ],
    lemon: [
        'Lemons contain more sugar than strawberries',
        'Add half a cup of lemon juice to your laundry to brighten whites'
    ],
    lime: [
        'Because of all the lime juice they would drink, British sea men were called limeys'
    ],
    orange: [
        'Oranges are the most commonly grown tree fruit in the world',
        'If you plant a single orange seed you will probably get more than one plant'
    ],
    pear: [
        'Pear trees are able to withstand temperatures as low as -40 degrees Celsius'
    ],
    watermelon: [
        'Watermelons are 92% water',
        'The largest watermelon ever grown was 262 pounds, or 119 KG',
        'Early explorers used watermelons as canteens to carry their water supplies'
    ],
    strawberry: [
        'The Romans used strawberries for a long list of medical remedies',
        'Strawberries were used as sacred symbols by Christian stonemasons',
        'The average strawberry has around 200 seeds',
        'Strawberries are the only fruit with their seeds on the outside'
    ],
    peach: [
        'Almonds are a member of the peach family',
        'Peaches are a member of the rose family',
        'Ripe peaches taste best when eaten at room temperature',
        'Nectarines are just peaches without the fuzz',
        'There are over 700 varieties of peaches'
    ],
    mango: [
        'Mangoes contain more vitamin C than oranges',
        'More fresh mangoes are eaten every day than any other fruit in the world',
        'Mangoes can be ripened quickly by being placed in a paper bag with a ripe banana'
    ],
    pineapple: [
        'Pineapples were once very rare. Thus the name King of Fruits',
        'Pineapples contain bromelain, which is used to stop cold beer from going cloudy',
        'Unripe pineapples are toxic',
        'The pineapple is not a single fruit, but a cluster of up to 200 fruitlets',
        "1/3 of the world's pineapples are produced in Hawaii"
    ],
    passion: [
        'The passion fruit flower is the national flower of Paraguay',
        'Passion fruits were first grown in Brazil',
        'Passion fruit is sweetest when slightly wrinkled',
        'The purple-fruited species of passion fruit self-fertilizes'
    ]
};

const fruits = [
    {id: 1, name: 'apple', background: 'lightgreen'},
    {id: 2, name: 'banana', background: 'lightyellow'},
    {id: 3, name: 'coconut', background: '#f0f0f0'},
    {id: 4, name: 'kiwi', background: 'green'},
    {id: 5, name: 'lemon', background: 'yellow'},
    {id: 6, name: 'lime', background: '#660054'},
    {id: 7, name: 'orange', background: 'orange'},
    {id: 8, name: 'pear', background: '#338f29'},
    {id: 9, name: 'watermelon', background: '#ff6c6c'},
    {id: 10, name: 'mango', background: '#f67b00'},
    {id: 11, name: 'pineapple', background: 'orangered'},
    {id: 12, name: 'passion', background: 'orangered'},
    {id: 13, name: 'strawberry', background: '#ce1010'}
];

/**
 *
 * @param {string} fruit
 * @return {string[]}
 * @throws {Error}
 */
export function getFruitFacts(fruit) {
    if (!fruitFacts[fruit]) {
        throw new Error("Invalid fruit name");
    }
    return fruitFacts[fruit];
}

export function findFruit(fruit) {
    const f = fruits.find(f => f.name === fruit);

    if (!f) {
        throw new Error("Invalid fruit name");
    }
    return f;
}

export function getFruits() {
    return fruits;
}
